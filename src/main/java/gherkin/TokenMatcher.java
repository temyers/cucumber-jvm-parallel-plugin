...
public class TokenMatcher implements gherkin.Parser.ITokenMatcher {
    private static final Pattern LANGUAGE_PATTERN = Pattern.compile("^\\s*#\\s*language\\s*:\\s*"
        + "([a-zA-Z\\-_]+)\\s*$");
    ...
    protected void setTokenMatched(Token token, gherkin.Parser.TokenType matchedType, String
        text, String keyword, Integer indent, List<GherkinLineSpan> items) {
        token.matchedType = matchedType;
        token.matchedKeyword = keyword;
        token.matchedText = text;
        token.mathcedItems = items;
        token.matchedGherkinDialect = getCurrentDialect();
        token.matchedIndent = indent != null ? indent : (token.line == null ? 0 : token.line
            .indent());
    ...
    public boolean match_EOF(Token token) {
        if (token.isEOF()) {
            setTokenMatched(token, gherkin.Parser.TokenType.EOF, null, null, null, null);
            return true;
        }
        return false;
    }
    ...
    public boolean match_Other(Token token) {
        String text = token.line.getLineText(indentToRemove); //take the entire line, except
        // removing DocString indents
        setTokenMatched(token, gherkin.Parser.TokenType.Other, unescapeDocString(text), null, 0,
            null);
        return true;
    }
    ...
    public boolean match_Empty(Token token) {
        if (token.line.isEmpty()) {
            setTokenMatched(token, gherkin.Parser.TokenType.Empty, null, null, null, null);
            return true;
        }
        return false;
    }
    ...
    public boolean match_Comment(Token token) {
        if (token.line.startsWith(GherkinLanguageConstants.COMMENT_PREFIX)) {
            ...
            setTokenMatched(token, gherkin.Parser.TokenType.Comment, text, null, 0, null);
            return true;
        }
        return false;
    }
    ...
    public boolean match_Language(Token token) {
        ...
        if (matcher.matches()) {
            ...
            setTokenMatched(token, gherkin.Parser.TokenType.Language, language, null, null, null);
    ...
    public boolean match_TagLine(Token token) {
        if (token.line.startsWith(GherkinLanguageConstants.TAG_PREFIX)) {
            setTokenMatched(token, gherkin.Parser.TokenType.TagLine, null, null, null, token.line
                .getTags());
            return true;
        }
        return false;
    ...
    public boolean match_FeatureLine(Token token) {
        return matchTitleLine(token, gherkin.Parser.TokenType.FeatureLine, currentDialect
            .getFeatureKeywords());
    }

    @Override
    public boolean match_BackgroundLine(Token token) {
        return matchTitleLine(token, gherkin.Parser.TokenType.BackgroundLine, currentDialect
            .getBackgroundKeywords());
    }

    @Override
    public boolean match_ScenarioLine(Token token) {
        return matchTitleLine(token, gherkin.Parser.TokenType.ScenarioLine, currentDialect
            .getScenarioKeywords());
    }

    @Override
    public boolean match_ScenarioOutlineLine(Token token) {
        return matchTitleLine(token, gherkin.Parser.TokenType.ScenarioOutlineLine, currentDialect
            .getScenarioOutlineKeywords());
    }

    @Override
    public boolean match_ExamplesLine(Token token) {
        return matchTitleLine(token, gherkin.Parser.TokenType.ExamplesLine, currentDialect
            .getExamplesKeywords());
    }

    private boolean matchTitleLine(Token token, gherkin.Parser.TokenType tokenType, List<String>
        keywords) {
        for (String keyword : keywords) {
            if (token.line.startsWithTitleKeyword(keyword)) {
                String title = token.line.getRestTrimmed(keyword.length()
                    + GherkinLanguageConstants.TITLE_KEYWORD_SEPARATOR.length());
    ...
    public boolean match_DocStringSeparator(Token token) {
        return activeDocStringSeparator == null
            ...
            || match_DocStringSeparator(token, GherkinLanguageConstants
                .DOCSTRING_ALTERNATIVE_SEPARATOR, true)
            // close
    ...
    private boolean match_DocStringSeparator(Token token, String separator, boolean isOpen) {
        if (token.line.startsWith(separator)) {
            ...
            setTokenMatched(token, gherkin.Parser.TokenType.DocStringSeparator, contentType,
                null, null, null);
            return true;
        }
        return false;
    ...
    public boolean match_StepLine(Token token) {
        ...
        for (String keyword : keywords) {
            if (token.line.startsWith(keyword)) {
                ...
                setTokenMatched(token, gherkin.Parser.TokenType.StepLine, stepText, keyword,
                    null, null);
                return true;
            }
        }
    ...
    public boolean match_TableRow(Token token) {
        if (token.line.startsWith(GherkinLanguageConstants.TABLE_CELL_SEPARATOR)) {
            setTokenMatched(token, gherkin.Parser.TokenType.TableRow, null, null, null, token
                .line.getTableCells());
            return true;
        }
        return false;
